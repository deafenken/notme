...
| 反检测 | `Function.prototype.toString` 可拆穿伪装，`data-geomirror` 属性残留在 DOM | 伪装函数对 `fn.toString()` 与 `Function.prototype.toString.call(fn)` 都显示 native；payload 改用 `window.postMessage` 传递，不写入 DOM |
...
GeoMirror 是标准的 Chromium MV3 扩展，Edge 的加载方式完全相同：

1. 下载或克隆本仓库。
2. 在地址栏输入并打开 `edge://extensions`。
3. 打开左下角的 **开发人员模式 / Developer mode**。
4. 点击 **加载解压缩的扩展 / Load unpacked**。
5. 选择 `geomirror` 文件夹（即包含 `manifest.json` 的那个目录）。
6. 从工具栏拼图图标里固定 GeoMirror，打开弹窗点击 **Refresh**。

Edge 支持 GeoMirror 用到的所有 `chrome.*` API（`declarativeNetRequest`、`alarms`、`privacy`），因此全部功能都可正常工作。如果弹窗显示错误，先确认代理/VPN 已连接，再点一次 **Refresh**。
...
- **反检测**：`Function.prototype.toString.call(fn)` 曾会返回 wrapper 源码；payload 曾以 `<html data-geomirror>` 明文留在 DOM；`permissions.query` 曾返回普通对象。均已修复。
