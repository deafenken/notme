...
- **Manual timezone (fallback)** — exit-IP detection is always tried first and wins when it succeeds. If it **fails** (or a provider returns no timezone), the timezone, locale, and a representative coordinate from this picked zone are used instead, so you're never left with the real timezone exposed. Leave it on *Auto only* to rely solely on detection.
